<?php
include '/usr/local/var/www/rafiki/web-app/vendor/autoload.php';
...
if (!isset($_SESSION["user"])){
  header("Location: http://localhost/rafiki/web-app/auth-login.php");
}
...
if(isset($_POST['change_profile'])){
    ...
    $conn->query($sql_emergency);  
$conn->close();
header("Location: http://localhost/rafiki/web-app/profile.php");
...
if (isset($_POST["post_publication"])){
  ...
  $uploadDir = $_SERVER['DOCUMENT_ROOT'].'/web-app/publication-uploads/'; // Directory where uploaded files will be stored
  $fileName = $_FILES['cover_image']['name'];
  $uploadFile = $uploadDir . $fileName;

  if(move_uploaded_file($_FILES['cover_image']['tmp_name'], $uploadFile)){
    $uploadDir1 = $_SERVER['DOCUMENT_ROOT'].'/web-app/content-uploads/'; // Directory where uploaded files will be stored
  ...
  VALUES ('$var1', '$uploadFile', '$var3','$uploadCont', '$var4','$currentDate','$id','$var5');
    ...
    echo "File upload failed with error: " . $error["message"];
    }
  }else{
   $error = error_get_last();
    echo "File upload failed with error: " . $error["message"];  
  ...
  header("Location: http://localhost/rafiki/web-app/publication-new.php");
...
if(isset($_POST["make_appointment"])){
  ...
  $conn->query($sql_statement);
  $conn->close();
header("Location: http://localhost/rafiki/web-app/appointment-new.php");
}
...
$conn->close();
header("Location: http://localhost/rafiki/web-app/patients-upload.php");
}
...
if(isset($_POST["change_hr_pass"])){
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      //$conn->query($sql_statement);
      //header("Location: http://localhost/rafiki/web-app/company-view.php?see_id=".$row["company_id"]);
